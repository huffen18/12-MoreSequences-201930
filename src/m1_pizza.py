...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 3:
    title = 'DRAW_POINTS_ON_CIRCLE, test 3:  30 cyan dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'black'
    draw_points_on_circle(window, circle, 30, 'cyan')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 1:
    title = 'PIZZA test 1:  200 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        piece = rg.Line(circle.center, points[k])
        piece.color = color
        piece.thickness = thickness
        piece.attach_to(window)
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    # Tests 3:
    title = 'POLYGON test 3:  6 segments with thin purple lines'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 6, 'purple', 2)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points) - 1):
        seg = rg.Line(points[k], points[k + 1])
        seg.thickness = thickness
        seg.color = color
        seg.attach_to(window)
    segf = rg.Line(points[number_of_segments - 1], points[0])
    segf.color = color
    segf.thickness = thickness
    segf.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 3:  600 cyan lines on purple circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'purple'
    fancy_polygon(window, circle, 300, 61, 'cyan', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points) - 1):
        start = points[k]
        end = points[k + hops_to_next_point % 2]
        if end is None:
            line = rg.Line(points[number_of_lines - 1], points[0])
            line.color = color
